...
function emptyState() {
  return {
    ...
    events: [],    // {id, name, date, flights, dinner, note, confirmed, playerIds}
...
function normalizePlayers(raw) {
  ...
  return (Array.isArray(raw) ? raw : []).map((p) => {
    ...
    if (!name) return null;
    return { id, name, hcp: M.normalizeHcp(p.hcp), present: p.present !== false };
  }).filter(Boolean);
}
...
function normalizeEvents(raw, playerIds) {
  const valid = new Set(playerIds);
  return (Array.isArray(raw) ? raw : []).map((e) => {
    ...
    return {
      ...
      playerIds: (Array.isArray(e.playerIds) ? e.playerIds : []).filter((pid) => valid.has(pid)),
    };
  }).filter(Boolean);
}
...
function normalizeRounds(raw) {
  return (Array.isArray(raw) ? raw : []).map((r) => {
    ...
    const scores = M.normalizeScores(r.scores, players.map((p) => p.id));
    ...
    if (storedResults.length && Object.keys(scores).length) {
      ...
      results = storedResults.map((x) => {
        ...
        return M.playerResult(player, scores[player.id]);
      });
      ...
      results = storedResults.map((x) => {
        ...
        const played = x.played == null ? M.HOLES : Number(x.played) || 0;
        return {
          id: str(x.id, 40) || newId(),
          name: str(x.name, 40),
          hcp: M.normalizeHcp(x.hcp),
          target: Number(x.target) || M.targetFor(x.hcp),
          gross: Number(x.gross) || 0,
          played,
          complete: played === M.HOLES,
          parOpen: 0,
      ...
      results = M.resultsFor(players, scores);
    }
