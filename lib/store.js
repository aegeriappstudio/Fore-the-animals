...
// 4 = `present` wieder da, 5 = Platz-Auswahl (courseId),
// 6 = offizielle Ratings: Geschlecht pro Spieler, Wertungs-Tees pro Runde
const SCHEMA = 6;
...
function emptyState() {
  return {
    ...
    tees: null,    // Wertungs-Tees {m, f} des aktiven Platzes (null = Platz-Standard)
    players: [],   // {id, name, hcp, gender, present}
...
function normalizePlayers(raw) {
  ...
  return (Array.isArray(raw) ? raw : []).map((p) => {
    ...
    return {
      ...
      gender: M.normalizeGender(p.gender), // Altbestand ohne Feld: Herren
      present: typeof p.present === 'boolean' ? p.present : null,
    };
  }).filter(Boolean);
}

// Wertungs-Tees {m, f} gegen die gerateten Tees des Platzes prüfen
function normalizeTees(raw, courseId) {
  const input = raw && typeof raw === 'object' ? raw : {};
  return {
    m: M.normalizeTee(input.m, 'm', courseId),
    f: M.normalizeTee(input.f, 'f', courseId),
  };
}
...
function normalizeRounds(raw) {
  return (Array.isArray(raw) ? raw : []).map((r) => {
    ...
    const tees = normalizeTees(r.tees, courseId);
    ...
    if (storedResults.length && Object.keys(scores).length) {
      ...
      results = storedResults.map((x) => {
        const player = { id: str(x.id, 40), name: str(x.name, 40), hcp: x.hcp, gender: x.gender };
        return M.playerResult(player, scores[player.id], courseId, tees);
      });
      ...
      results = M.resultsFor(players, scores, courseId, tees);
    }
...
function migrate(raw) {
  ...
  state.tees = normalizeTees(input.tees, state.courseId);
