...
// 1 = Urversion, 2 = rounds/rev, 3 = ohne `present` und Termin-Anmeldungen
const SCHEMA = 3;
...
function normalizePlayers(raw) {
  ...
  return (Array.isArray(raw) ? raw : []).map((p) => {
    ...
    if (!name) return null;
    // Ein `present`-Feld gibt es nicht mehr – wer heute spielt, ergibt sich
    // aus der Flight-Zuteilung (siehe model.todaysPlayers).
    return { id, name, hcp: M.normalizeHcp(p.hcp) };
  }).filter(Boolean);
}
