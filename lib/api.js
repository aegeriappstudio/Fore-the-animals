...
function createApi(store, options) {
  ...
  async function handle(req, res, url) {
    ...
    if (method === 'PUT' && url.pathname === '/api/course') {
      ...
      const course = M.courseById(str(body.courseId, 20));
      if (!course) return json(res, 400, { error: 'Unbekannter Platz' });
      if (course.id !== state.courseId && M.hasAnyScore(state.scores)) {
        // Mit Scores auf 9 Löchern auf 18 wechseln (oder umgekehrt) würde die
        // laufende Runde entwerten – zuerst abschliessen oder zurücksetzen.
      ...
      return json(res, 200, { courseId: state.courseId, rev: state.rev });
    }
    ...
    if (method === 'POST' && url.pathname === '/api/rounds') {
      ...
      store.commit((s) => {
        ...
        round = {
          ...
          courseId: s.courseId,
          results: M.resultsFor(players, s.scores, s.courseId),
