...
const { newId, str, isDate, normalizeTeeTime } = require('./store.js');
...
function createApi(store, options) {
  ...
  async function handle(req, res, url) {
    ...
    if (method === 'POST' && url.pathname === '/api/players') {
      ...
      const player = { id: newId(), name, hcp: M.normalizeHcp(body.hcp), present: true };
      store.commit((s) => {
    ...
    if (seg(1) === 'players' && seg(2)) {
      ...
      if (method === 'DELETE') {
        ...
        store.commit((s) => {
          ...
          s.flights.forEach((f) => { f.playerIds = f.playerIds.filter((pid) => pid !== player.id); });
          s.events.forEach((ev) => { ev.playerIds = ev.playerIds.filter((pid) => pid !== player.id); });
        });
    ...
    if (method === 'POST' && url.pathname === '/api/flights/randomize') {
      ...
      const present = M.presentPlayers(state.players);
      if (present.length < 2) return json(res, 400, { error: 'Mindestens 2 anwesende Spieler nötig' });
      store.commit((s) => {
      ...
      if (method === 'PUT') {
        ...
        store.commit((s) => {
          ...
          if (Array.isArray(body.playerIds)) {
            const allowed = new Set(M.presentPlayers(s.players).map((p) => p.id));
            const ids = [];
    ...
    if ((method === 'PUT' || method === 'POST') && url.pathname === '/api/scores') {
      ...
      store.commit((s) => {
        entries.forEach((raw) => {
          ...
          const hole = M.normalizeHole(raw && raw.hole);
          if (!findPlayer(s, playerId)) {
            return rejected.push({ playerId, hole, error: 'Spieler nicht gefunden' });
          }
          if (hole === null) {
            return rejected.push({ playerId, hole: raw && raw.hole, error: `Loch muss 1–${M.HOLES} sein` });
          }
          ...
          const merged = M.mergeEntry(s.scores[playerId][hole], raw, hole);
    ...
    if (method === 'PUT' && seg(1) === 'scores' && seg(2) && seg(3)) {
      ...
      const hole = M.normalizeHole(seg(3));
      if (!findPlayer(state, playerId)) return json(res, 404, { error: 'Spieler nicht gefunden' });
      if (hole === null) return json(res, 400, { error: `Loch muss 1–${M.HOLES} sein` });
      const body = await readBody(req);
      ...
      store.commit((s) => {
        if (!s.scores[playerId]) s.scores[playerId] = {};
        const merged = M.mergeEntry(s.scores[playerId][hole], body, hole);
        if (merged.gross === null && Object.keys(merged.animals).length === 0) delete s.scores[playerId][hole];
        else s.scores[playerId][hole] = merged;
    ...
    if (method === 'POST' && url.pathname === '/api/rounds') {
      ...
      store.commit((s) => {
        ...
        // sonst stünde ein anwesender, aber nicht gestarteter Spieler mit der
        // reinen Par-Prognose in der Schlussrangliste.
        const players = M.presentPlayers(s.players).filter((p) => M.hasScores(s.scores[p.id]));
        round = {
          id: newId(),
          name: str(body.name, 60) || defaultName,
          date: date.toISOString(),
          eventId: str(body.eventId, 40) || null,
          results: M.resultsFor(players, s.scores),
    ...
    if (seg(1) === 'events' && seg(2)) {
      ...
      // An-/Abmelden steht allen offen (keine PIN)
      if (method === 'POST' && seg(3) === 'signup') {
        const body = await readBody(req);
        const player = findPlayer(state, str(body.playerId, 40));
        if (!player) return json(res, 404, { error: 'Spieler nicht gefunden' });
        ...
        return json(res, 200, { ok: true, playerIds: ev.playerIds, rev: state.rev });
      }

      if (!requirePin(req, res)) return;

      if (method === 'POST' && seg(3) === 'apply-attendance') {
        let present = 0;
        store.commit((s) => {
          ...
          s.players.forEach((p) => {
            ...
            else s.flights.forEach((f) => { f.playerIds = f.playerIds.filter((pid) => pid !== p.id); });
          });
        });
        return json(res, 200, { ok: true, present, rev: state.rev });
      }
