...
function createApi(store, options) {
  ...
  async function handle(req, res, url) {
    ...
    if (method === 'POST' && url.pathname === '/api/players') {
      ...
      const player = { id: newId(), name, hcp: M.normalizeHcp(body.hcp) };
      store.commit((s) => {
    ...
    if (seg(1) === 'players' && seg(2)) {
      ...
      if (method === 'PUT') {
        ...
        const changed = store.commit((s) => {
          ...
          // `flightId` weist den Spieler einem Flight zu (null = spielt heute
          // nicht mit). Damit ersetzt eine Aktion die frühere Kombination aus
          // Anwesenheits-Schalter und Flight-Zuteilung.
    ...
    if (method === 'POST' && url.pathname === '/api/flights/randomize') {
      ...
      if (Array.isArray(body.playerIds) && body.playerIds.length) {
        candidates = body.playerIds.filter((pid) => known.has(pid));
      } else {
        candidates = M.todaysPlayers(state.players, state.flights, state.scores).map((p) => p.id);
        if (!candidates.length) candidates = state.players.map((p) => p.id);
      }
      if (candidates.length < 2) return json(res, 400, { error: 'Mindestens 2 Spieler nötig' });
      store.commit((s) => {
        ...
        store.commit((s) => {
          ...
          if (Array.isArray(body.playerIds)) {
            ...
            body.playerIds.forEach((pid) => {
              if (known.has(pid) && ids.indexOf(pid) === -1) ids.push(pid);
            });
